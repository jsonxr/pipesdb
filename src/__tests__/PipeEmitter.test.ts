...
describe('PipeEmitter', () => {
  it('constructor', () => {
    const emitter = new PipeEmitter<Example>();
  ...
  it('should dispatch a function', () => {
    const emitter = new PipeEmitter<Example>();
    const ev: PipeSyncEvent<Example> = { type: 'updated', key: '1', value: { key: '1', name: 'one' } };
    const mock = jest.fn();
  ...
  it('should unsubscribe', () => {
    ...
    // Dispatch 1st event
    const ev1: PipeSyncEvent<Example> = { type: 'updated', key: '1', value: { key: '1', name: 'one' } };
    emitter.emit(ev1);
    unsubscribe();

    // Dispatch 2nd event
    const ev2: PipeSyncEvent<Example> = { ...ev1, key: '2' };
    emitter.emit(ev2);
